...
# -*- coding: utf-8 -*-
"""
Step 04 (variant A): MinHash + LSH near-duplicate dedup on QUESTION only.

Input:
- jsonl with ShareGPT-ish schema:
  {"id":..., "conversations":[{"from":"human","value":...},{"from":"gpt","value":...}], "meta": {...}}

Output (under --output_dir):
- kept.jsonl: first-seen unique samples
- dropped.jsonl: near-duplicates with reason, matched_id, estimated_jaccard
- summary.json: counts + top_department breakdown
...
def main() -> None:
    ...
    with open(args.input_jsonl, "r", encoding="utf-8") as fin, kept_path.open("w", encoding="utf-8") as fk, dropped_path.open(
        ...
        it = tqdm(fin, total=total_lines, desc="MinHash dedup (question)", unit="lines")
